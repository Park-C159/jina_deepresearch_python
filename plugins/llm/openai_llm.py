"""OpenAI / 兼容 OpenAI 协议的 LLM 插件"""
import os
from typing import Any, Dict

import openai

from core.plugin_protocols import BaseLLMPlugin
from core.plugin_registry import register_plugin
from config.config_loader import config


_OPENAI_VALID_KEYS = {
    "api_key", "base_url", "timeout", "max_retries", "default_headers",
    "default_query", "http_client", "_strict_response", "organization", "project"
}


@register_plugin("llm", "openai")
class OpenAILLMPlugin(BaseLLMPlugin):
    """OpenAI 模型插件。也兼容所有 OpenAI-API 兼容端点（如 vLLM、LocalAI）。"""

    def initialize(self, cfg: Dict[str, Any]) -> None:
        self.api_key = cfg.get("api_key") or os.getenv("OPENAI_API_KEY")
        self.base_url = cfg.get("base_url") or os.getenv("OPENAI_BASE_URL")
        self.provider_cfg = config["providers"].get("openai", {})
        self.models_cfg = config["models"].get("openai", {})
        if not self.api_key:
            raise RuntimeError("OPENAI_API_KEY not found")

    def health_check(self) -> bool:
        try:
            client = self.get_raw_client()
            client.models.list()
            return True
        except Exception:
            return False

    def get_tool_config(self, tool_name: str) -> Dict[str, Any]:
        default = self.models_cfg.get("default", {})
        overrides = self.models_cfg.get("tools", {}).get(tool_name, {})
        return {
            "model": overrides.get("model") or default.get("model", "gpt-4o"),
            "temperature": overrides.get("temperature") if overrides.get("temperature") is not None else default.get("temperature", 0),
            "maxTokens": overrides.get("maxTokens") or default.get("maxTokens", 8192),
        }

    def get_client(self, tool_name: str) -> tuple:
        cfg = self.get_tool_config(tool_name)
        raw_cfg = self.provider_cfg.get("clientConfig") or {}
        client_kwargs = {"api_key": self.api_key}
        client_kwargs.update({k: v for k, v in raw_cfg.items() if k in _OPENAI_VALID_KEYS})
        if self.base_url:
            client_kwargs["base_url"] = self.base_url
        compatibility = raw_cfg.get("compatibility")
        return openai.OpenAI(**client_kwargs), compatibility, cfg["model"]

    def get_raw_client(self) -> openai.OpenAI:
        return openai.OpenAI(api_key=self.api_key, base_url=self.base_url)
