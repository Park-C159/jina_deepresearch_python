...
from utils.schemas import QueryRewriterSchema


def get_prompt(query, think, context):
...
The current time is {iso_str}. Current year: {current_year}, current month: {current_month}.
...
5. Temporal Context: Add a time-sensitive query that incorporates the current date ({current_year}-{current_year}) to ensure recency and freshness of information.
...
# 假设这些类型和常量已在别处定义
# from your_module import SearchAction, TrackerContext, Schemas, SERPQuery, ObjectGeneratorSafe, TOOL_NAME, get_prompt, log_info, log_error
...
async def rewrite_query(
    ...
    """
    将 action 中的 searchRequests 并发重写为 SERPQuery 列表。
    """
    try:
        ...
        async def _rewrite_one(req):
            ...
            if trackers and trackers.actionTracker:
                trackers.actionTracker.track_think(result["object"]["think"])
        ...
        logging.error("Query rewrite error:", {"error": error})
        raise
